...
class SegmentMeanPooling(nn.Module):
    def __init__(self):
        super(SegmentMeanPooling, self).__init__()

    def forward(self, feat, id_list):

        bag_sorted_list = torch.sort(id_list)  # 获取有序id
        zero_tensor = torch.tensor([0]).to(feat.device)
        seg_flag_tensor = (
            torch.nonzero(torch.diff(bag_sorted_list.values)).flatten() + 1
        )
        offsets = torch.cat((zero_tensor, seg_flag_tensor))
...
class PointColorPooling(nn.Module):
    def __init__(self):
        super(PointColorPooling, self).__init__()

    def forward(self, feat, rgbs, mode="mean"):

        unique_rgbs, inverse_indices = torch.unique(
        ...
        bag_feat = nn.functional.embedding_bag(
            input=bag_sorted_list.indices, weight=feat, offsets=offsets, mode=mode
        )
        new_feat = bag_feat[inverse_indices]
...
class MixedPooling(nn.Module):
    def __init__(self, compose):
        ...
        for com in compose:
            self.pool.append(MODELS.build(com))

    def forward(self, feat, id_list):
        feats = []
        for pool in self.pool:
            feats.append(pool(feat, id_list))
        return sum(feats)


@MODELS.register_module()
class FeatureFusion(nn.Module):
    def __init__(self, input_dim=64, hidden_dim=512, output_dim=64):
        super(FeatureFusion, self).__init__()

        self.meanpool = SegmentMeanPooling()
        self.colorpool = PointColorPooling()
        self.fc1 = nn.Linear(input_dim * 2, hidden_dim)
        self.fc2 = nn.Linear(hidden_dim, hidden_dim)
        self.fc3 = nn.Linear(input_dim + hidden_dim, output_dim)

    def forward(self, feat, id_list, rgbs):
        entity_avg_pool = self.meanpool(feat, id_list)
        avg_pool = self.meanpool(self.colorpool(feat, rgbs), id_list)
        max_pool = self.meanpool(self.colorpool(feat, rgbs, mode="max"), id_list)
        layerf = torch.cat((avg_pool, max_pool), dim=1)
        layerf = self.fc1(layerf)
        layerf = F.relu(layerf)
        layerf = self.fc2(layerf)

        fusion = torch.cat([entity_avg_pool, layerf], dim=1)
