...
@TRANSFORMS.register_module()
class ElasticTransfrom2D(BaseTransform):
    """Apply elastic augmentation to a 3D scene. Required Keys:
    Args:
        gran (List[float]): Size of the noise grid (in same scale[m/cm]
            as the voxel grid).
    ...
    def elastic(self, x, gran, mag):
        ...
        for blur in [blur0, blur1, blur2, blur0, blur1, blur2]:
            # Changed
            noise[0] = scipy.ndimage.filters.convolve(
                noise[0], blur, mode="constant", cval=0
            )
            noise[1] = scipy.ndimage.filters.convolve(
                noise[1], blur, mode="constant", cval=0
            )
            # noise[2] remains unchanged
        ax = [np.linspace(-(b - 1) * gran, (b - 1) * gran, b) for b in noise_dim]
        interp = [
            scipy.interpolate.RegularGridInterpolator(
                ax, n, bounds_error=0, fill_value=0
        ...
        # Changed
        # Apply noise only to the x and y dimensions
        x_noise = interp[0](x)
        y_noise = interp[1](x)
        z_noise = np.zeros_like(x_noise)  # No noise in z dimension
        # Changed
        return (
            x + np.hstack([x_noise[:, None], y_noise[:, None], z_noise[:, None]]) * mag
        )
...
class AddSuperPointAnnotations(BaseTransform):
    ...
    def __init__(self, num_classes, stuff_classes, merge_non_stuff_cls=True):
    ...
    def transform(self, input_dict):
        ...
        if insts_mask.shape[1] != 0:
            ...
            sp_masks_inst = scatter_mean(insts_mask.float(), sp_pts_mask, dim=-1)
        ...
        sem_mask = torch.nn.functional.one_hot(
            sem_mask, num_classes=self.num_classes + 1
        )
...
@TRANSFORMS.register_module()
class FloorscanAddSuperPointAnnotations(BaseTransform):
    """Prepare ground truth markup for training.
...
class PointInstClassMapping_(BaseTransform):
    ...
    def transform(self, input_dict):
        ...
        if self.structured3d:
            ...
            pts_instance_mask[pts_semantic_mask == 0] = pts_instance_mask.max() + 1
            # floor as one instance
            pts_instance_mask[pts_semantic_mask == 1] = pts_instance_mask.max() + 1
...
class PointSample_(PointSample):
    ...
    def transform(self, input_dict):
        ...
        points, choices = self._points_random_sampling(points, self.num_points)
...
@TRANSFORMS.register_module()
class FloorplanPointSample_(PointSample):
    ...
    def transform(self, input_dict):
        ...
        pts_instance_mask = input_dict.get("pts_instance_mask", None)
        pts_semantic_mask = input_dict.get("pts_semantic_mask", None)
        pts_reverse_map = input_dict.get("pts_reverse_map", None)
        ...
        if pts_semantic_mask is not None:
            pts_semantic_mask = pts_semantic_mask[choices]
            input_dict["pts_semantic_mask"] = pts_semantic_mask
        if pts_reverse_map is not None:
            pts_reverse_map = pts_reverse_map[choices]

            idxs = np.unique(pts_reverse_map)
            mapping = np.zeros(np.max(idxs) + 2, dtype=int)
            new_idxs = np.arange(len(idxs))
            if idxs[0] == -1:
                mapping[idxs] = new_idxs - 1
                ...
                mapping[idxs] = new_idxs
            pts_reverse_map = mapping[pts_reverse_map]

            input_dict["pts_reverse_map"] = pts_reverse_map
...
@TRANSFORMS.register_module()
class FloorplanPointSample_PTv3(FloorplanPointSample_):
    ...
    def _points_random_sampling(self, points, num_samples):
        ...
        mask = (points.coord[:, 0] <= 140) & (points.coord[:, 1] <= 140)
        ...
        return points[intersection], intersection
    
@TRANSFORMS.register_module()
class FloorplanPixelSample_PTv3(FloorplanPointSample_):
    ...
    def _points_random_sampling(self, points, num_samples):
        ...
        mask = (points.coord[:, 0]>=0 ) & (points.coord[:, 1]>=0 )
        # 首先，创建一个与 mask 等长的索引数组
        indices = np.arange(len(mask))
        # 然后，找出 mask 为 True 的索引位置
        mask_indices = indices[mask]
