...
@MODELS.register_module()
class InterpolateQueryDecoder(QueryDecoder):
    def __init__(
        self,
        num_layers,
        num_instance_queries,
        ...
        **kwargs,
        ...
        self.num_queries = num_interpolate + num_semantic_queries

    @staticmethod
    def baryrand(n: int, k: int, dtype=torch.float32, device=None):
        """
        Uniform sampling in k-dimensional unit hypervolume,


        Args:
            n (int): number of samples to draw
            k (int): dimension of the hypervolume

        Returns:
            Tensor: barycentric coordinates in hypervolume
        """
        x, _ = torch.sort(torch.rand((n, k - 1), dtype=dtype, device=device), dim=1)
        ret = torch.diff(
            x,
            dim=1,
            prepend=torch.zeros((n, 1), device=device),
            append=torch.ones((n, 1), device=device),
        )
        return ret
    ...
    def _get_queries(self, queries=None, batch_size=None):
        ...
        assert hasattr(self, "query") and (
            queries is None
        ), "Cannot handle query selection yet"
        ...
        for i in range(batch_size):
            result_query = []
            if hasattr(self, "query"):
                # barycentric sampling on instance queries
                bary = self.baryrand(
                    self.num_interpolate,
                    self.num_instance_queries,
                    device=self.query.weight.device,
                )
                queries_inst = bary @ self.query.weight[: self.num_instance_queries]

                # take semantic queries from the back of query weight directly
                queries_sem = self.query.weight[-self.num_semantic_queries :]
                result_query.append(torch.cat([queries_inst, queries_sem]))
            if queries is not None:
                result_query.append(self.query_proj(queries[i]))
            result_queries.append(torch.cat(result_query))
        return result_queries
...
@MODELS.register_module()
class Floors3dOneDataQueryDecoder(BaseModule):
    """Query decoder. The same as above, but for 2 datasets.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.num_queries_dataset = num_queries_dataset

        self.queries_dataset = nn.Embedding(num_queries_dataset, d_model)
        ...
        for i in range(num_layers):
            ...
            self.ffn_layers.append(FFN(d_model, hidden_dim, dropout, activation_fn))
        self.out_norm = nn.LayerNorm(d_model)
        self.out_cls_dataset = nn.Sequential(
            nn.Linear(d_model, d_model),
            nn.ReLU(),
            nn.Linear(d_model, num_classes_dataset + 1),
        )
        self.out_score = nn.Sequential(
            nn.Linear(d_model, d_model), nn.ReLU(), nn.Linear(d_model, 1)
        )
    ...
    def _get_queries(self, batch_size, scene_names):
        ...
        result_queries = []
        for i in range(batch_size):
            if self.prefix_dataset in scene_names[i]:
                result_queries.append(self.queries_dataset.weight)
            else:
                raise RuntimeError(f'Invalid scene name "{scene_names[i]}".')
    ...
    def _forward_head(self, queries, mask_feats, scene_names):
        ...
        for i in range(len(queries)):
            ...
            if self.prefix_dataset in scene_names[i]:
                cls_preds.append(self.out_cls_dataset(norm_query))
            else:
                raise RuntimeError(f'Invalid scene name "{scene_names[i]}".')
...
@MODELS.register_module()
class QueryDecoder_GELU(BaseModule):
    """Query decoder.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.objectness_flag = objectness_flag
        self.input_proj = nn.Sequential(
            nn.Linear(in_channels, d_model), nn.LayerNorm(d_model), nn.GELU()
        )
        self.num_queries = num_instance_queries + num_semantic_queries
        if num_instance_queries + num_semantic_queries > 0:
            self.query = nn.Embedding(
        ...
        if num_instance_queries == 0:
            self.query_proj = nn.Sequential(
                nn.Linear(in_channels, d_model), nn.GELU(), nn.Linear(d_model, d_model)
            )
        self.cross_attn_layers = nn.ModuleList([])
        self.self_attn_layers = nn.ModuleList([])
        self.ffn_layers = nn.ModuleList([])
        ...
        self.out_norm = nn.LayerNorm(d_model)
        self.out_cls = nn.Sequential(
            nn.Linear(d_model, d_model), nn.GELU(), nn.Linear(d_model, num_classes + 1)
        )
        if objectness_flag:
            self.out_score = nn.Sequential(
