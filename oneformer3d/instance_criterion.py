...
def dice_loss(inputs, targets):
    ...
    loss = 1 - (numerator + 1) / (denominator + 1)
    # is_nan = bool(torch.any(torch.isnan(loss.mean())))
    # if is_nan:
    #     print("!!!NANANAN")
    return loss.mean()
...
class InstanceCriterion:
    ...
    def get_layer_loss(self, aux_outputs, insts, indices=None):
        ...
        if indices is None:
            ...
            for i in range(len(insts)):
                ...
                gt_instances = InstanceData_(
                    labels=insts[i].labels_3d, masks=insts[i].sp_masks
                )
        ...
        if len(mask_bce_losses):
            ...
            mask_bce_loss = 0
            mask_dice_loss = 0
        # if bool(torch.any(torch.isnan(mask_bce_loss))):
        #     print('nan')
        # if bool(torch.any(torch.isnan(mask_dice_loss))):
        #     print('nan')
        # if len(pred_masks) == 0:
        #     print("LEN pred_mask == 0")
        # is_nan = bool(torch.any(torch.isnan(torch.stack(mask_dice_losses).sum())))
        # if is_nan:
        #     print("DICE SUM == 0")
        # is_nan = bool(torch.any(torch.isnan(torch.stack(mask_bce_losses).sum())))
        # if is_nan:
        #     print("BCE SUM == 0")
        # is_nan = (
        #     bool(torch.any(torch.isnan(mask_bce_loss))) if mask_bce_loss != 0 else False
        # )
        # if is_nan:
        #     print("BCE NAN")
        # is_nan = (
        #     bool(torch.any(torch.isnan(mask_dice_loss)))
        #     if mask_dice_loss != 0
        #     else False
        # )
        # if is_nan:
        ...
        #     score_loss,
        # )
        # print("__layer_loss__ inst_loss", loss)
    ...
    def __call__(self, pred, insts):
        ...
        # print("__call__ inst_loss", loss)
        return {"inst_loss": loss}


@MODELS.register_module()
class FloorplanInstanceCriterion(InstanceCriterion):
    def __init__(
        self,
        matcher,
        loss_weight,
    ...
    def get_layer_loss(self, aux_outputs, insts, indices=None):
        ...
        for cls_pred, inst, (idx_q, idx_gt) in zip(cls_preds, insts, indices):
            ...
            if cls_pred.shape[1] == self.num_classes_1dataset + 1:
                ...
                raise RuntimeError(f"Invalid classes number {cls_pred.shape[1]}.")
...
@MODELS.register_module()
class Floors3dOneDataCriterion:
    """Loss module for SPFormer.
    ...
    def __init__(
        ...
        self.class_weight_dataset = [1] * num_classes_dataset + [non_object_weight]
        self.loss_weight = loss_weight
        self.fix_dice_loss_weight = fix_dice_loss_weight
        self.iter_matcher = iter_matcher
    ...
    def get_layer_loss(self, aux_outputs, insts, indices=None):
        ...
        for cls_pred, inst, (idx_q, idx_gt) in zip(cls_preds, insts, indices):
            ...
            cls_target[idx_q] = inst.labels_3d[idx_gt]
            if cls_pred.shape[1] == self.num_classes_dataset + 1:
                cls_losses.append(
                    F.cross_entropy(
                        cls_pred,
                        cls_target,
                        cls_pred.new_tensor(self.class_weight_dataset),
                    )
                )
            else:
                raise RuntimeError(f"Invalid classes number {cls_pred.shape[1]}.")
