...
def compute_averages(aps, pr_rc, options, class_labels):
    ...
    if options and 'ap75' in options.keys() and options['ap75']:
        o75 = np.where(np.isclose(options['overlaps'], 0.75))
        o_all_but75 = np.where(
            np.logical_not(np.isclose(options['overlaps'], 0.75)))
        avg_dict = {}
        avg_dict['all_ap'] = np.nanmean(aps[d_inf, :, o_all_but75])
        avg_dict['all_ap_50%'] = np.nanmean(aps[d_inf, :, o50])
        avg_dict['all_ap_75%'] = np.nanmean(aps[d_inf, :, o75])
        avg_dict['classes'] = {}
        for (li, label_name) in enumerate(class_labels):
            ...
            avg_dict['classes'][label_name]['ap50%'] = np.average(aps[d_inf, li,
                                                                    o50])
            avg_dict['classes'][label_name]['ap75%'] = np.average(aps[d_inf, li,
                                                                    o75])
        return avg_dict
