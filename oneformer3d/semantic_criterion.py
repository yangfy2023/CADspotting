...
class S3DISSemanticCriterion:
    ...
    def __init__(
        self,
        loss_weight,
        seg_loss=dict(type="mmdet.CrossEntropyLoss", use_sigmoid=True),
    ):
    ...
    def get_layer_loss(self, layer, aux_outputs, insts):
        ...
        for pred_mask, gt_mask in zip(pred_masks, insts):
            seg_loss = self.seg_loss(pred_mask.T, gt_mask.sp_masks.float().argmax(0))
...
@MODELS.register_module()
class FloorplanSemanticCriterion:
    """Semantic criterion for S3DIS.
...
@MODELS.register_module()
class FloorscanSemanticCriterion:
    """Semantic criterion for ScanNet.
    ...
    def __init__(self, ignore_index, loss_weight, weight):
        self.ignore_index = ignore_index
        self.loss_weight = loss_weight
        self.weight = weight
        ...
        for pred_mask, gt_mask in zip(pred["sem_preds"], insts):
            ...
            losses.append(
                F.cross_entropy(
                    pred_mask,
                    gt_mask.sp_masks.float().argmax(0),
                    weight=torch.tensor(self.weight, device="cuda"),
                    ignore_index=self.ignore_index,
                )
            )
        loss = self.loss_weight * torch.mean(torch.stack(losses))
