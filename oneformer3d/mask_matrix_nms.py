...
def mask_matrix_nms_cpu(
    ...
    keep_inds = sort_inds.cpu()
    if nms_pre > 0 and len(sort_inds) > nms_pre:
        sort_inds = sort_inds[:nms_pre]
        keep_inds = keep_inds[:nms_pre]
        scores = scores[:nms_pre].cpu()
    masks = masks[sort_inds].cpu()
    mask_area = mask_area[sort_inds].cpu()
    labels = labels[sort_inds].cpu()

    num_masks = len(labels)
    flatten_masks = masks.reshape(num_masks, -1).float()
    # inter.
    inter_matrix = torch.mm(flatten_masks, flatten_masks.transpose(1, 0)).cpu()
    expanded_mask_area = mask_area.expand(num_masks, num_masks).cpu()
    # Upper triangle iou matrix.
    iou_matrix = (
        inter_matrix
    ...
    # update the score.
    scores = scores.cpu() * decay_coefficient
    ...
    return scores.cuda(), labels.cuda(), masks.cuda(), keep_inds.cuda()