...
def rename_gt(gt_semantic_masks, gt_instance_masks, valid_class_ids):
    ...
    for semantic_mask, instance_mask in zip(gt_semantic_masks,
                                            gt_instance_masks):
        semantic_mask = np.array(semantic_mask, copy=True)
        instance_mask = np.array(instance_mask, copy=True)
...
def instance_seg_eval(gt_semantic_masks,
    ...
    if options and 'ap75' in options.keys() and options['ap75']:
        header = ['classes', 'AP_0.75', 'AP_0.50', 'AP']
        rows = []
        for label, data in metrics['classes'].items():
            aps = [data['ap75%'], data['ap50%'], data['ap']]
            rows.append([label] + [f'{ap:.4f}' for ap in aps])
        aps = metrics['all_ap_75%'], metrics['all_ap_50%'], metrics['all_ap']
