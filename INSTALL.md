...
```bash
conda create -n new_oneformer3d python=3.10
conda activate new_oneformer3d
```
...
```bash
conda install -n new_oneformer3d -y pkg-config cmake cairo git ninja
```

## 3. Install PyTorch cu121

```bash
conda run -n new_oneformer3d python -m pip install \
...
```bash
conda run -n new_oneformer3d env MMCV_WITH_OPS=1 FORCE_CUDA=1 \
...
```bash
conda run -n new_oneformer3d python -m pip install mmdet3d==1.4.0
```

## 6. Install CUDA extensions used by the default PTv3 path

```bash
conda run -n new_oneformer3d python -m pip install spconv-cu121
```
...
```bash
conda run -n new_oneformer3d python -m pip install -r requirements.txt
```
...
```bash
conda run -n new_oneformer3d env PYTHONPATH=$(pwd) \
  python -c "import torch, mmengine, mmdet3d, oneformer3d; print('env ok')"
```

```bash
conda run -n new_oneformer3d env PYTHONPATH=$(pwd) python tools/train.py --help
conda run -n new_oneformer3d env PYTHONPATH=$(pwd) python tools/test.py --help
conda run -n new_oneformer3d python -m pip check
```
