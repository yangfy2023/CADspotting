...
class FloorplanData(object):
    ...
    def __init__(self, root_path, split="train", scannet200=False, save_path=None):
        ...
        self.cat_ids = np.array(list(range(5, 37)))
        ...
        split_file = osp.join(self.root_dir, "meta_data", f"floorplan_{split}.txt")
    ...
    def get_aligned_box_label(self, idx):
        box_file = osp.join(
            self.root_dir, "floorplan_instance_data", f"{idx}_aligned_bbox.npy"
        )
    ...
    def get_unaligned_box_label(self, idx):
        box_file = osp.join(
            self.root_dir, "floorplan_instance_data", f"{idx}_unaligned_bbox.npy"
        )
        mmengine.check_file_exist(box_file)
        return np.load(box_file)
    ...
    def get_axis_align_matrix(self, idx):
        matrix_file = osp.join(
            self.root_dir, "floorplan_instance_data", f"{idx}_axis_align_matrix.npy"
        )
    ...
    def get_infos(self, num_workers=4, has_label=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            pts_filename = osp.join(
                self.root_dir, "floorplan_instance_data", f"{sample_idx}_vert.npy"
            )
            points = np.load(pts_filename)
            mmengine.mkdir_or_exist(osp.join(self.save_path, "points"))
            points.tofile(osp.join(self.save_path, "points", f"{sample_idx}.bin"))
            info["pts_path"] = osp.join("points", f"{sample_idx}.bin")

            sp_filename = osp.join(
                self.root_dir, "floorplan_instance_data", f"{sample_idx}_sp_label.npy"
            )
            ...
            if not self.test_mode:
                ...
                mmengine.mkdir_or_exist(osp.join(self.save_path, "instance_mask"))
                mmengine.mkdir_or_exist(osp.join(self.save_path, "semantic_mask"))
