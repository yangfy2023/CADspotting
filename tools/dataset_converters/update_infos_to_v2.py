...
def generate_nuscenes_camera_instances(info, nusc):
    ...
    for cam in camera_types:
        ...
        ann_infos = get_nuscenes_2d_boxes(
            nusc, cam_info["sample_data_token"], visibilities=["", "1", "2", "3", "4"]
        )
...
def update_nuscenes_infos(pkl_path, out_dir):
    ...
    for i, ori_info_dict in enumerate(mmengine.track_iter_progress(data_list["infos"])):
        ...
        for ori_sweep in ori_info_dict["sweeps"]:
            ...
            temp_lidar_sweep["lidar_points"]["lidar2ego"] = (
        ...
        for cam in ori_info_dict["cams"]:
            ...
            empty_img_info["img_path"] = Path(
                ori_info_dict["cams"][cam]["data_path"]
            ).name
...
def update_kitti_infos(pkl_path, out_dir):
    ...
    for ori_info_dict in mmengine.track_iter_progress(data_list):
        ...
        temp_data_info["images"]["CAM0"]["lidar2img"] = (
            ori_info_dict["calib"]["P0"] @ lidar2cam
        ).tolist()
        temp_data_info["images"]["CAM1"]["lidar2img"] = (
            ori_info_dict["calib"]["P1"] @ lidar2cam
        ).tolist()
        temp_data_info["images"]["CAM2"]["lidar2img"] = (
            ori_info_dict["calib"]["P2"] @ lidar2cam
        ).tolist()
        temp_data_info["images"]["CAM3"]["lidar2img"] = (
            ori_info_dict["calib"]["P3"] @ lidar2cam
        ).tolist()

        temp_data_info["lidar_points"]["Tr_velo_to_cam"] = Trv2c.tolist()

        # for potential usage
        temp_data_info["images"]["R0_rect"] = (
            ori_info_dict["calib"]["R0_rect"].astype(np.float32).tolist()
        )
        temp_data_info["lidar_points"]["Tr_imu_to_velo"] = (
            ori_info_dict["calib"]["Tr_imu_to_velo"].astype(np.float32).tolist()
        )
        ...
        if anns is not None:
            ...
            for instance_id in range(num_instances):
                ...
                empty_instance["bbox"] = anns["bbox"][instance_id].tolist()
                empty_instance["truncated"] = anns["truncated"][instance_id].tolist()
                ...
                empty_instance["difficulty"] = anns["difficulty"][instance_id].tolist()
            ...
            if anns["gt_num"] == 0:
                ...
                for instance_id in range(num_instances):
                    ...
                    if anns["class"][instance_id] < len(METAINFO["classes"]):
                        empty_instance["bbox_label_3d"] = anns["class"][instance_id]
                    else:
...
def update_s3dfloorplan_infos(pkl_path, out_dir):
    print(f"{pkl_path} will be modified.")
    if out_dir in pkl_path:
        print(f"Warning, you may overwriting " f"the original data {pkl_path}.")
        time.sleep(5)
    ...
    for i, ori_info_dict in enumerate(mmengine.track_iter_progress(data_list)):
        ...
        if "pts_reverse_map_path" in ori_info_dict:
            temp_data_info["pts_reverse_map_path"] = Path(
                ori_info_dict["pts_reverse_map_path"]
            ).name
            ...
            if anns["gt_num"] == 0:
                ...
                for instance_id in range(num_instances):
                    ...
                    if anns["name"][instance_id] in METAINFO["classes"]:
                        ...
                        empty_instance["bbox_label"] = empty_instance["bbox_label_3d"]
                    else:
                        ignore_class_name.add(anns["name"][instance_id])
                        empty_instance["bbox_label_3d"] = -1
                        empty_instance["bbox_label"] = -1
...
def update_waymo_infos(pkl_path, out_dir):
    ...
    for ori_info_dict in mmengine.track_iter_progress(data_list):
        ...
        for cam_idx, cam_key in enumerate(camera_types):
            temp_data_info["images"][cam_key]["cam2img"] = ori_info_dict["calib"][
                f"P{cam_idx}"
            ].tolist()
        ...
        for cam_idx, cam_key in enumerate(camera_types):
            ...
            temp_data_info["images"][cam_key]["lidar2img"] = (
                ori_info_dict["calib"][f"P{cam_idx}"] @ lidar2cam
            ).tolist()
        ...
        temp_data_info["lidar_points"]["timestamp"] = ori_info_dict["timestamp"]
        ...
        if anns is not None:
            ...
            for instance_id in range(num_instances):
                ...
                rots = anns["rotation_y"][:, None][instance_id]
                gt_bboxes_3d = (
                    np.concatenate([loc, dims, rots]).astype(np.float32).tolist()
                )
                empty_instance["bbox_3d"] = gt_bboxes_3d
                empty_instance["bbox_label_3d"] = copy.deepcopy(
                    empty_instance["bbox_label"]
        ...
        if anns is not None:
            ...
            cam_instances = generate_waymo_camera_instances(ori_info_dict, camera_types)
            temp_data_info["cam_instances"] = cam_instances
...
def parse_args():
    ...
    parser.add_argument("--dataset", type=str, default="kitti", help="name of dataset")
    parser.add_argument(
        "--pkl-path",
        type=str,
...
def update_pkl_infos(dataset, out_dir, pkl_path):
    ...
    elif dataset.lower() == "s3dfloorplan":
        update_s3dfloorplan_infos(pkl_path=pkl_path, out_dir=out_dir)
    else:
...
if __name__ == "__main__":
    ...
    update_pkl_infos(dataset=args.dataset, out_dir=args.out_dir, pkl_path=args.pkl_path)
