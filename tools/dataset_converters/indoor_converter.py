...
from .floorplan_data_utils import (
    S3DFloorplanData,
    S3DFloorplanSegData,
)
...
def create_indoor_info_file(
    ...
    elif pkl_prefix == "s3dfloorplan":
        # S3DIS doesn't have a fixed train-val split
        # it has 6 areas instead, so we generate info file for each of them
        # in training, we will use dataset to wrap different areas
        splits = [f"Area_{i}" for i in [1, 2, 3, 4]]
        for split in splits:
            dataset = S3DFloorplanData(root_path=data_path, split=split)
            info = dataset.get_infos(num_workers=workers, has_label=True)
            filename = os.path.join(save_path, f"{pkl_prefix}_infos_{split}.pkl")
            mmengine.dump(info, filename, "pkl")
            print(f"{pkl_prefix} info {split} file is saved to {filename}")
            seg_dataset = S3DFloorplanSegData(
                data_root=data_path,
                ann_file=filename,
                split=split,
                num_points=4096,
