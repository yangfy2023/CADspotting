...
class S3DFloorplanData(object):
    ...
    def __init__(self, root_path, split="Area_1"):
        ...
        self.sample_id_list = os.listdir(
            osp.join(self.data_dir, split)
        )  # conferenceRoom_1
    ...
    def get_infos(self, num_workers=4, has_label=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            pts_reverse_map_path = osp.join(
                self.root_dir, "s3dis_data", f"{self.split}_{sample_idx}_reverse_map.npy"
            )
            ...
            pts_reverse_map = np.load(pts_reverse_map_path).astype(np.int64)
            ...
            mmengine.mkdir_or_exist(osp.join(self.root_dir, "reverse_map"))

            points.tofile(
                osp.join(self.root_dir, "points", f"{self.split}_{sample_idx}.bin")
            )
            pts_instance_mask.tofile(
                osp.join(
                    self.root_dir, "instance_mask", f"{self.split}_{sample_idx}.bin"
                )
            )
            pts_semantic_mask.tofile(
                osp.join(
                    self.root_dir, "semantic_mask", f"{self.split}_{sample_idx}.bin"
                )
            )
            pts_reverse_map.tofile(
                osp.join(
                    self.root_dir, "reverse_map", f"{self.split}_{sample_idx}.bin"
                )
            )
            ...
            info["pts_reverse_map_path"] = osp.join(
                "reverse_map", f"{self.split}_{sample_idx}.bin"
            )
...
class S3DFloorplanSegData(object):
    ...
    def __init__(
        ...
        self.all_ids = np.arange(36)  # all possible ids
        self.cat_ids = np.array(
            [
                0,
                1,
